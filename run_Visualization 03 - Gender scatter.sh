#!/usr/bin/env bash
# Runner for "Visualization 03 - Gender scatter.ipynb" -- portable execute + debug + control-visibility patch.
set -euo pipefail
cd "$(dirname "$0")"
if [[ -n "${PYTHON:-}" ]]; then
  PY="$PYTHON"
...
NB="Visualization 03 - Gender scatter.ipynb"
OUT="Visualization 03 - Gender scatter_out.ipynb"
