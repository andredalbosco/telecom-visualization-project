#!/usr/bin/env bash
# Runner for "Visualization 02 - Heatmap region.ipynb" -- portable execute + debug + control-visibility patch.
set -euo pipefail
cd "$(dirname "$0")"
if [[ -n "${PYTHON:-}" ]]; then
  PY="$PYTHON"
...
NB="Visualization 02 - Heatmap region.ipynb"
OUT="Visualization 02 - Heatmap region_out.ipynb"
